...
extern const int __tezzt_registered_signals [];
...
#define BEGIN_TEST_GROUP(NAME)                  \
 void __testgroup_##NAME##__(void) {            \
  ...
  __attribute__((unused)) static const char * __group_name__ = #NAME;   \
  static unsigned passed_tests = 0;             \
  static unsigned failed_tests = 0;             \
                                                \
  atexit(__tezzt__exit_program_with_report);    \
  for(int i=0; __tezzt_registered_signals[i]>0; ++i) \
     signal(__tezzt_registered_signals[i],  __tezzt__signal_handler);     \
